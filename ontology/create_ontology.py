...
'''
aspect_categories = {"movie": ["music", "acting", "plot", "direction"], "music": ["genre", "lyrics", "artist"]}
...
def extract_concepts(node, node_limit):
    ...
    for syn_p in synonyms:
        ...
        for e in tree['edges']:
            try:
                if e['end']['@type'] == "Node" and e['start']['@type'] == "Node" and e['start']['language'] == "en" and \
                        e['end']['language'] == "en":
                    ...
                    for ch in child.split():
                        ch = ch.lower()
                        if ch not in stopwords and ch not in ['a', 'an', 'the'] and ps.stem(syn_p) != ps.stem(
                                ch) and ch not in concepts:
                            score[ch] = float(score[syn_p]) / 2 + float(
                                e['weight'])  # both conceptnet "weight" and half of parent's score
                            concepts.append(ch)
                            parent[ch] = syn_p
            except:
                pass
